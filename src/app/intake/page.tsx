...
const completionChecks: Record<string, boolean> = {
  'client-details': Boolean(formData.fullName && formData.dateOfBirth && formData.nationality && formData.residenceCountry && formData.email && formData.phone),
  'migration-goal': Boolean(formData.mainGoal && formData.interestedCountry && formData.timeframe),
  family: Boolean(formData.maritalStatus && formData.dependants && formData.migrateWithFamily),
  education: Boolean(formData.highestQualification && formData.fieldOfStudy && formData.institution && formData.studyCountry && formData.completionYear),
  employment: Boolean(formData.currentOccupation && formData.migrationOccupation && formData.workExperienceYears && formData.currentEmployer && formData.dutiesSummary),
  ...
  'risk-screening': Boolean(formData.previousRefusal && formData.previousCancellation && formData.overstayRemoval && formData.criminalHistory && formData.healthCondition),
...
<div className="hero"><h1>VPM Intake Questionnaire</h1><p>Please complete all sections for a preliminary migration assessment.</p><p className="draft-status">{draftStatus}</p><p className="draft-status">Last saved: {lastSavedAt ? new Date(lastSavedAt).toLocaleString() : 'Not saved yet'}</p><button type="button" className="secondary-btn" onClick={clearSavedDraft}>Clear saved draft</button></div>
...
<Fieldset id="english-language" title="English" helper="English test details support eligibility and points claims."><Select label="English test completed" value={formData.englishTestCompleted} options={['Yes','No']} onChange={(v) => onChange('englishTestCompleted', v)} /><Input label="Test type" value={formData.englishTestType} onChange={(v) => onChange('englishTestType', v)} /><Input label="Test date" type="date" value={formData.englishTestDate} onChange={(v) => onChange('englishTestDate', v)} /><Input label="Score summary" value={formData.englishScoreSummary} onChange={(v) => onChange('englishScoreSummary', v)} /></Fieldset>
<Fieldset id="risk-screening" title="Risk screening" helper="We need risk disclosures early for accurate advice."><Select label="Previous visa refusal" value={formData.previousRefusal} options={['Yes','No']} onChange={(v) => onChange('previousRefusal', v)} /><Input label="Refusal details" value={formData.refusalDetails} onChange={(v) => onChange('refusalDetails', v)} /><Select label="Previous visa cancellation" value={formData.previousCancellation} options={['Yes','No']} onChange={(v) => onChange('previousCancellation', v)} /><Select label="Overstay/removal history" value={formData.overstayRemoval} options={['Yes','No']} onChange={(v) => onChange('overstayRemoval', v)} /><Select label="Criminal history" value={formData.criminalHistory} options={['Yes','No']} onChange={(v) => onChange('criminalHistory', v)} /><Input label="Character details" value={formData.characterDetails} onChange={(v) => onChange('characterDetails', v)} /><Select label="Serious health condition" value={formData.healthCondition} options={['Yes','No']} onChange={(v) => onChange('healthCondition', v)} /><Input label="Health details" value={formData.healthDetails} onChange={(v) => onChange('healthDetails', v)} /></Fieldset>
